...
class TradingEngine:
    ...
    def __init__(self, stock_list: List[str], mode: str = "AI", alert_callback=None, activity_callback=None):
        ...
        self.max_workers = max(1, min(2, int(os.environ.get("SCAN_WORKERS", "2"))))
        ...
        twilio_sid    = self._clean_env("TWILIO_ACCOUNT_SID")
        twilio_token  = self._clean_env("TWILIO_AUTH_TOKEN")
        self.twilio_from   = self._clean_env("TWILIO_FROM_NUMBER")
        self.twilio_to     = self._clean_env("TWILIO_TO_NUMBER")  # your cell number
    ...
    def _apply_confirmed_risk_profile(self, settings: Dict[str, Any]) -> None:
        ...
        # Hard-guard limits scale evenly with the slider (5% per level): level 1 is
        # most restricted (55% exposure / 45% reserve), level 5 is medium (75%/25%),
        # and level 10 is fully unrestricted (100%/0%) — which makes the exposure/
        # reserve HARD GUARDS impossible to trip at 10 (a buy can never push
        # exposure above 100% or reserve below 0%). Stop-loss, trailing stop, and
        # never-sell-below-cost are unaffected — only the capital-deployment guard
        # relaxes.
        self.max_gross_exposure_pct = min(100.0, max(10.0, float(profile["max_gross_exposure_pct"])))
        self.min_cash_reserve_pct  = min(90.0, max(0.0, float(profile["min_cash_reserve_pct"])))
    ...
    def _send_twilio_call(self, spoken_message: str):
        ...
        try:
            ...
            safe_message = escape(spoken_message)
            twiml = (
                f"<Response><Say voice='alice'>{safe_message} "
                f"This message will repeat once.</Say>"
                f"<Pause length='1'/>"
                f"<Say voice='alice'>{safe_message}</Say></Response>"
            )
    ...
    def _get_bars_df(self, symbol: str, limit: int = 100) -> Optional[pd.DataFrame]:
        ...
        if cached and (time.time() - cached[1]) < self._bars_cache_ttl:
            # A cached None is intentional negative caching after a rate-limit or
            # unavailable-data response. Do not hammer Alpaca again next cycle.
            return cached[0]
        ...
        # Cache both successes and failures. A failed request is negatively cached
        # so dozens of symbols do not immediately repeat the same quota-consuming
        # bars/hourly-bars/fallback sequence on the next scan cycle.
        self._bars_cache[symbol] = (df if df is not None and not df.empty else None, time.time())
        return df
