...
def write_keys_sh(keys: dict):
    """Write keys.sh for Mac/Linux users (sourced before launching the app)."""
    lines = [
        "#!/bin/bash",
        "# --------------------------------------------------------",
        "#  Alien AI Trader -- Environment Variables (Mac/Linux)",
        "#  Generated by setup_wizard.py",
        "#  Usage: source keys.sh",
        "#  DO NOT share or commit this file!",
        "# --------------------------------------------------------",
        "",
    ]
    ...
    with open("keys.sh", "w", encoding="utf-8") as f:
        f.write("\n".join(lines))
...
{C.RESET}
  {C.WHITE}Windows:{C.RESET}
    1. Double-click {C.YELLOW}START.bat{C.RESET}  (or the Desktop shortcut)
    2. Open {C.CYAN}http://localhost:5000{C.RESET}

  {C.WHITE}Mac / Linux:{C.RESET}
    1. Run: {C.YELLOW}source keys.sh && python dashboard.py{C.RESET}
    2. Open {C.CYAN}http://localhost:5000{C.RESET}

  {C.WHITE}Render (cloud):{C.RESET}
    1. Add the environment variables shown above
       to BOTH your web and worker services
    2. Trigger a manual deploy

  {C.WHITE}Your cloud dashboard:{C.RESET}
    {C.CYAN}https://alien-ai-trader-dashboard.onrender.com{C.RESET}

  {C.WHITE}Verify your installation:{C.RESET}
    Run {C.YELLOW}python self_test.py{C.RESET} (or double-click {C.YELLOW}SELF_TEST.bat{C.RESET})
